...
function applyWaveShaper(buffer: Float32Array, curve: Float32Array): void {
...
function biquadHPF(freq: number, sampleRate: number, Q: number): BiquadCoeffs {
...
function biquadPeaking(freq: number, sampleRate: number, Q: number, gainDB: number): BiquadCoeffs {
  const w0 = (2 * Math.PI * freq) / sampleRate;
  const cos = Math.cos(w0);
...
function biquadHighShelf(freq: number, sampleRate: number, gainDB: number): BiquadCoeffs {
  const w0 = (2 * Math.PI * freq) / sampleRate;
  const cos = Math.cos(w0);
  const A = 10 ** (gainDB / 40);
...
function applyPultecStyle(buffer: Float32Array, sampleRate: number, lowContourAmount: number): void {
...
function applyLimiter(buffer: Float32Array, sampleRate: number, ceilingDB: number, attackMs: number = 5): void {
...
function hyperCompress(buffer: Float32Array, threshold: number = 0.3, ratio: number = 4): void {
  for (let i = 0; i < buffer.length; i++) {
    const x = buffer[i];
    const abs = Math.abs(x);
...
function applyNeuroDrive(buffer: Float32Array, sampleRate: number): void {
  const wet = 0.22;
  const dry = 1 - wet;
...
function processMonoChannel(channel: Float32Array, sampleRate: number, params: MasteringParams): void {
...
export function buildMasteringChain(left: Float32Array, right: Float32Array, sampleRate: number, params: MasteringParams): void {
...
export function measureLUFS(left: Float32Array, right: Float32Array, sampleRate: number): number {
  let sum = 0;
